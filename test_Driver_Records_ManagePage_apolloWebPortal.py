#               Apollo Web Portal - DRIVER RECORDS

def Driver_Records_ManagePage (driver, driver_Name, driver_Version, carrier):
    ...
    button_Driver_Records_ManagePageAdr = test_Global_Addresses_apolloWebPortal.button_Driver_Records_ManagePageAdr_Global
    title_Driver_Records_Driver_RecordsPageAdr = test_Global_Addresses_apolloWebPortal.title_Driver_Records_Driver_RecordsPageAdr_Global
...
#   Page Title on TCReport
    function_Name = "DRIVER RECORDS page"
...
#       Function Log Books side bar button click
    ...
    button_Driver_Records_ManagePage = WebDriverWait(menuRegion_MainSideBar_ManagePageAsideDiv, 10).until(EC.element_to_be_clickable((By.XPATH, button_Driver_Records_ManagePageAdr))).click()
    time.sleep(3)

#       Title of the right page
    title_Driver_Records_Driver_RecordsPage = w.until(EC.presence_of_element_located((By.XPATH, title_Driver_Records_Driver_RecordsPageAdr))).text
    test_Name = "Driver Records page open"
    condition1 = "Driver Records"
    condition2 = title_Driver_Records_Driver_RecordsPage
    tc_reports.assert_test_reportfile(test_Name, condition1, condition2, driver, driver_Name)
    #       Print Assert OK
    print_Information_Fix = "Driver Records Page - Driver Records page open:"
    print_Information_Var = title_Driver_Records_Driver_RecordsPage
    test_Case_Type = ""
    contol_Reportfile = 1
